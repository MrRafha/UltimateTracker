...
const FEATURES = [
  {
    ...
    desc: 'Visualize em tempo real os nodes ativos do servidor com marcadores coloridos por tipo de recurso.',
  },
  {
    icon: '⏱️',
    title: 'Timers ao vivo',
    desc: 'Countdowns precisos para spawn de cada node, com filtros por tempo e tipo de recurso.',
  },
  {
    icon: '🤖',
    title: 'Bot Discord',
    desc: 'Reporte nodes direto do Discord com /scout — o bot exibe no mapa da sua guilda instantaneamente.',
  },
  {
    icon: '🛡️',
    title: 'Por guilda',
    desc: 'Cada guilda tem seu próprio tracker privado. Apenas membros com a role configurada têm acesso.',
  },
];

const COMMANDS = [
  { name: '/setup', desc: 'Registra o servidor e configura o canal de tracking.' },
  { name: '/role', desc: 'Define a role que pode ver e reportar nodes.' },
  { name: '/scout', desc: 'Reporta um node: zona, objetivo e duração.' },
  { name: '/mapa', desc: 'Envia o link do mapa da sua guilda no chat.' },
];
    ...
    <div style={{ minHeight: '100vh', background: '#0d0d0d', fontFamily: 'system-ui, sans-serif', color: '#e0e0e0' }}>
      ...
      <section style={{
        ...
        <div style={{
          ...
          Albion Online · Tracking de Nodes
        </div>

        <h1 style={{ fontSize: 52, fontWeight: 900, lineHeight: 1.1, margin: '0 0 20px', color: '#fff', maxWidth: 700 }}>
          Seu mapa de gathering,<br />
          <span style={{ color: '#5865F2' }}>em tempo real.</span>
        </h1>
        <p style={{ fontSize: 17, color: '#777', maxWidth: 500, lineHeight: 1.6, margin: '0 0 40px' }}>
          Adicione o bot ao seu servidor Discord, configure sua guilda e tenha um mapa colaborativo de nodes com timers precisos.
        </p>
      ...
      <section style={{ padding: '60px 32px', maxWidth: 720, margin: '0 auto' }}>
        ...
        <p style={{ textAlign: 'center', color: '#555', fontSize: 14, marginBottom: 40 }}>
          Tudo via slash commands — sem prefixos, sem configuração complicada.
        </p>
      ...
      <footer style={{
        ...
        <div style={{ display: 'flex', gap: 20 }}>
          <Link href="/pricing" style={{ color: '#555', textDecoration: 'none' }}>Preços</Link>
          <Link href="/login" style={{ color: '#555', textDecoration: 'none' }}>Entrar</Link>
        </div>
